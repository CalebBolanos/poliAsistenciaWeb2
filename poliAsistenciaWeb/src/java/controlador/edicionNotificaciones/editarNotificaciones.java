 ...
 * @author Caleb
 */
@WebServlet(name = "editarNotificaciones", urlPatterns = {"/notificaciones/editarNotificaciones"})
public class editarNotificaciones extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String idNotificacion = request.getParameter("idNoti") != null ? request.getParameter("idNoti") : "";
        if (usr.getAttribute("idpersona") == null || usr.getAttribute("idtipo") == null) {
            response.sendRedirect("iniciarSesion");
            return;
        }
        try (PrintWriter out = response.getWriter()) {
            int idNoti = Integer.parseInt(idNotificacion);
                ...
                if (tipoPersona == 3) {//profesor
                    ...
                    if (tipoPersona == 4) {//jefe de academia
                        ...
                        if (tipoPersona == 1) {//gestion
                            ...
                            String numero = (String) usr.getAttribute("numero");
                            String nombre = (String) usr.getAttribute("nombre");
                            gestion.editarNotificaciones persona = new gestion.editarNotificaciones(idPersona, tipoPersona, numero, nombre);
                            out.print(persona.dibujarEditorNotificaciones(idNoti));
                        }
                    }
                }
                out.flush();
