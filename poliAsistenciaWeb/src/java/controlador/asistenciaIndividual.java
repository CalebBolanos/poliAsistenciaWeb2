 ...
 * @author Caleb
 */
@WebServlet(name = "asistenciaIndividual", urlPatterns = {"/asistenciaIndividual"})
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (tipoPersona == 2) {//alumno
                ...
                alumno.asistenciaIndividual alumno = new alumno.asistenciaIndividual(idPersona, tipoPersona, boleta, nombre);
                out.print(alumno.dibujarAsistenciaIndividual());
            } else {
                if (tipoPersona == 3) {//profesor
                    int idPersona = Integer.parseInt((String) usr.getAttribute("idpersona"));
                    String numero = (String) usr.getAttribute("numero");
                    String nombre = (String) usr.getAttribute("nombre");
                    profesor.inicio profesor = new profesor.inicio(idPersona, tipoPersona, numero, nombre);
                    out.print(profesor.dibujarInicio());
                } else {
                    if (tipoPersona == 4) {//jefe de academia
                        int idPersona = Integer.parseInt((String) usr.getAttribute("idpersona"));
                        String numero = (String) usr.getAttribute("numero");
                        String nombre = (String) usr.getAttribute("nombre");
                        jefeAcademia.inicio jefe = new jefeAcademia.inicio(idPersona, tipoPersona, numero, nombre);
                        out.print(jefe.dibujarInicio());
                    }
                    else{
